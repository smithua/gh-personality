...
class JPagination extends JObject
{
    ...
    public function getPagesLinks()
    {
        ...
        if($component=='com_content' && $view=='article' && !$app->isAdmin())return $this->getPagesLinksP3000();
        else return $this->getPagesLinksJoomla();
    }

    public function getPagesLinksJoomla()
    {
        $app = JFactory::getApplication();

        // Build the page navigation list.
        if(defined('SH404SEF_IS_RUNNING')) $data = $this->_buildDataObjectSEF404();
        else $data = $this->_buildDataObject();
    ...
    public function getPagesLinksP3000()
    {
        $doc = JFactory::getDocument();

        // Build the page navigation list.
        if(defined('SH404SEF_IS_RUNNING')) $data = $this->_buildDataObjectSEF404(true);
        else $data = $this->_buildDataObject(true);

        if ($this->total <= $this->limit) return '';

        // Adding JS and CSS files
        $assetsPath = JURI::root() . 'plugins/system/jpaginator3000/assets/';
        $doc->addScript($assetsPath . 'paginator3000.js');
        $doc->addStyleSheet($assetsPath . 'paginator3000.css');

        // Build JS pages
        $js = 'var paginator3000Pages = [';
        $pagesArray = array();
        ...
        $js .= implode(',',$pagesArray) . "];";
        $pageCount = $this->get('pages.total');
        $pagesShow = $pageCount<5 ? 5 : round($pageCount / 3);
        $js .= "document.addEvent('domready', function(){
                paginator = new Paginator('paginator3000'," . $pageCount . "," . $pagesShow  . "," . $data->currentPage . ");
                });";

        $doc->addScriptDeclaration($js);
        return '<div id="paginator3000" class="paginator"></div>';
    }
     ...
     *
     * @param bool $buildAllLinks build all links for paginator3000
     *
     * @return  object  Pagination data object.
     *
     * @since   11.1
     */
    protected function _buildDataObject($buildAllLinks = false)
    {
        // Initialise variables.
        $data = new stdClass;
        ...
        $data->pages = array();
        $stop = !$buildAllLinks ? $this->get('pages.stop') : $this->get('pages.total');
        $i = !$buildAllLinks ? $this->get('pages.start') : 1;
        //$currentPage = !$buildAllLinks ? $this->get('pages.current') : $this->get('pages.current') + $this->get('pages.start')-1;
        ...
        for ($i ; $i <= $stop; $i++)
        {
            ...
            else if ($i ==  $this->get('pages.current')) $data->currentPage = $i;
        }
        return $data;
    }

    protected function _buildDataObjectSEF404($buildAllLinks=false)
    {
        // Initialise variables.
        $data = new stdClass;
        ...
        $data->pages = array();
        $stop = !$buildAllLinks ? $this->get('pages.stop') : $this->get('pages.total');
        $start = !$buildAllLinks ? $this->get('pages.start') : 1;
        ...
        $data->currentPage  = $this->get('pages.current');
        return $data;
    }
}
