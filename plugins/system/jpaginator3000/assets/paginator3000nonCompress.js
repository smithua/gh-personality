 ...
 */
Paginator = function(id, pagesTotal, pagesSpan, currentPage, pages, returnOrder){
...
Paginator.prototype.initPages = function(){
    ...
    html += "</tr>" +
        ...
        "</div>" +
        "</td></tr>"+
        "<tr><td colspan='" + this.pagesSpan + "'><span style='font-size: 9px;color:#808080;'>Powered by <a style='font-size: 9px;color:#808080;' href='http://joomalungma.com' target='_blank'>Joomalungma</a>.</span></td></tr>"
    "</table>"
...
//		this.dx = getMousePosition(e).x - this.xPos;
    ...
    if ('function' == typeof window.addEvent) window.addEvent('resize',function(){resizePaginator();})
    else addEvent(window, 'resize', resizePaginator);
...
//		if (tagName != 'INPUT' && tagName != 'TEXTAREA' && !event.alt && event.control) {
        ...
        if (event.ctrlKey && event.keyCode == 37) {
            if (_this.currentPage > 1) {
                window.location.href = _this.pages[_this.currentPage - 1];
            }
...
Paginator.prototype.drawPages = function(){
    ...
    for(var i= 0; i < this.pagesCells.length; i++){
        var currentCellValue = this.firstCellValue + i - 1;
        if(currentCellValue == this.currentPage){
            html = "<span>" + "<em>" + String(currentCellValue + 1) + "</em></span>";
        } else {
            html = "<span>" + "<a href='" + this.pages[currentCellValue] + "'>" + String(currentCellValue + 1) + "</a></span>";
        }
...
if ('function' != typeof window.addEvent)
{
