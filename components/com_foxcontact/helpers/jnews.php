...
class FJNewsSubscriber extends FNewsletter
{
	...
	protected function load_newsletter_lists()
	{
		...
		$query->select($db->$GLOBALS["quoteName"]("id") . "," . $db->$GLOBALS["quoteName"]("hidden") . " as " . $db->$GLOBALS["quoteName"]("visible") . "," . $db->$GLOBALS["quoteName"]("list_name") . " as " . $db->$GLOBALS["quoteName"]("name"));  // JNews "hidden" means "visible"

		$query->from($db->$GLOBALS["quoteName"]("#__jnews_lists"));
		...
		$query->where($db->$GLOBALS["quoteName"]("id") . " IN (" . implode(',', $lists) .")");
